(() => {
    ...
    class SelectConstructor {
        ...
        getSelectElementContent(selectOption) {
            const selectOptionData = selectOption.dataset.asset || selectOption.dataset.icon ? `${selectOption.dataset.icon || selectOption.dataset.asset}` : "";
            const selectOptionDataHTML = selectOptionData ? `<img src="${selectOptionData}" alt="" class="option-icon">` : "";
            let selectOptionContentHTML = ``;
            selectOptionContentHTML += `<span class="${this.selectClasses.classSelectRow}">`;
            selectOptionContentHTML += `<span class="${this.selectClasses.classSelectData}">${selectOptionDataHTML}</span>`;
            selectOptionContentHTML += `<span class="${this.selectClasses.classSelectText}">${selectOption.textContent}</span>`;
        ...
        optionAction(selectItem, originalSelect, optionItem) {
            ...
            if (!selectOptions.classList.contains("_slide")) {
                if (originalSelect.multiple) {
                    ...
                    const targetUrl = optionItem.dataset.href;
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        icons.forEach((icon => {
            icon.addEventListener("click", (function() {
                const selectItem = icon.closest(".select");
                if (selectItem) modules_flsModules.select.selectAction(selectItem);
            }));
        }));
    }));
    class ScrollWatcher {
    ...
    Parallax.Each = class {
        ...
        animationFrame() {
            ...
            if (positionParent.top > heightWindow || positionParent.bottom < 0) {
                this.animationID = window.requestAnimationFrame(this.animation);
                return;
            }
